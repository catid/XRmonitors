...
MonitorHologramMain::MonitorHologramMain(
...
, Desktops(cameras)
{
    ...
    if (m_canGetDefaultHolographicDisplay)
    {
        ...
        m_holographicDisplayIsAvailableChangedEventToken = HolographicSpace::IsAvailableChanged(bind(&MonitorHologramMain::OnHolographicDisplayIsAvailableChanged, this, _1, _2));
    }
...
void MonitorHologramMain::SetHolographicSpace(
    HWND hWnd, 
    HolographicSpace const& holographicSpace)
{
	...
	Desktops.Initialize(m_deviceResources);
    ...
    m_cameraAddedToken = m_holographicSpace.CameraAdded(std::bind(&MonitorHologramMain::OnCameraAdded, this, _1, _2));
    ...
    m_cameraRemovedToken = m_holographicSpace.CameraRemoved(std::bind(&MonitorHologramMain::OnCameraRemoved, this, _1, _2));
...
void MonitorHologramMain::UnregisterHolographicEventHandlers()
{
...
HolographicFrame MonitorHologramMain::Update(HolographicFramePrediction& prediction)
{
    // Before doing the timer update, there is some work to do per-frame
    // to maintain holographic rendering. First, we will get information
    // about the current frame.

	Desktops.UpdateUiSettings();

	Desktops.UpdateMonitorEnumeration();
	...
	// Work around issue: https://github.com/MicrosoftDocs/winrt-api/issues/494
	// We must wait for frame to finish!
	// So this means we should not call WaitForNextFrameReady
    ...
    {
        winrt::Windows::Perception::PerceptionTimestamp timestamp = prediction.Timestamp();
        const auto location = m_spatialLocator.TryLocateAtTimestamp(
            timestamp,
            m_stationaryReferenceFrame.CoordinateSystem());

        Desktops.UpdateHeadPose(location);
    }
	...
	m_timer.Tick([this]()
    {
        ...
        Desktops.UpdateSimulation(m_timer);
    });
        ...
        for (HolographicCameraPose const& cameraPose : prediction.CameraPoses())
        {
            ...
            if (m_stationaryReferenceFrame != nullptr)
            {
                renderingParameters.SetFocusPoint(
                    m_stationaryReferenceFrame.CoordinateSystem(),
                    m_shaderMonitorRenderer->GetPosition()
                );
            }
#endif
        }
...
void MonitorHologramMain::OnDoff(bool doff)
{
	Desktops.OnDoff(doff);
}

// Renders the current frame to each holographic camera, according to the
// current application and spatial positioning state. Returns true if the
// frame was rendered to at least one camera.
bool MonitorHologramMain::Render(
	...
	HolographicFramePrediction const& prediction)
{
    ...
    // Grab latest desktop images, re-enumerate if needed
    Desktops.UpdateDesktopDuplication();
	    ...
	    [this, holographicFrame](std::map<UINT32, std::unique_ptr<DX::CameraResources>>& cameraResourceMap)
	{
		...
		for (HolographicCameraPose const& cameraPose : prediction.CameraPoses())
		{
			...
			// Prepare to render with quad layers
			auto& holocam = cameraPose.HolographicCamera();
			xrm::QuadLayersTracker layer_tracker;
			layer_tracker.Camera = &holocam;
			layer_tracker.Frame = &holographicFrame;
			layer_tracker.Context = m_deviceResources->GetD3DDeviceContext();
			layer_tracker.Start();

			Desktops.Render(cameraPose, layer_tracker, pCameraResources);

			const uint64_t t1 = GetTimeUsec();

			if (t1 - t0 > 2000) {
				Logger.Info("Render: ", (t1 - t0), " usec");
			}

			layer_tracker.End();

			pCameraResources->ResolveSubresource(m_deviceResources.get());
		}
...
void MonitorHologramMain::SaveAppState()
{
    //
...
void MonitorHologramMain::LoadAppState()
{
    //
...
void MonitorHologramMain::OnPointerPressed()
{
}

void MonitorHologramMain::OnHotkey(int index, bool down)
{
	Desktops.OnHotkey(index, down);
}

// Notifies classes that use Direct3D device resources that the device resources
// need to be released before this method returns.
void MonitorHologramMain::OnDeviceLost()
{
	Desktops.ReleaseVrResources();
}

// Notifies classes that use Direct3D device resources that the device resources
// may now be recreated.
void MonitorHologramMain::OnDeviceRestored()
{
	Desktops.CreateVrResources();
}

void MonitorHologramMain::OnLocatabilityChanged(SpatialLocator const& sender, winrt::Windows::Foundation::IInspectable const& /*args*/)
{
...
void MonitorHologramMain::OnCameraAdded(
...
void MonitorHologramMain::OnCameraRemoved(
    HolographicSpace const& /*sender*/,
...
void MonitorHologramMain::OnHolographicDisplayIsAvailableChanged(winrt::Windows::Foundation::IInspectable, winrt::Windows::Foundation::IInspectable)
{
    ...
    if (m_spatialLocator != spatialLocator)
    {
        ...
        if (spatialLocator != nullptr)
        {
            ...
            m_locatabilityChangedToken = m_spatialLocator.LocatabilityChanged(std::bind(&MonitorHologramMain::OnLocatabilityChanged, this, _1, _2));
