...
namespace DX
{
    ...
    public:
		...
		// My addons -cat
		uint32_t GetMaxQuadLayerCount();
		bool IsHardwareContentProtectionSupported();
		bool IsHardwareContentProtectionEnabled();
		bool IsPrimaryLayerEnabled();
		uint32_t GetQuadLayerCount();
		void SetQuadLayerSize(uint32_t index, unsigned width, unsigned height);
		uint32_t AddQuadLayer(unsigned width, unsigned height);
		void TruncateQuadLayers(uint32_t count);
		HolographicQuadLayer GetQuadLayer(uint32_t index);
		void ResolveSubresource(DX::DeviceResources* pDeviceResources); // MSAA
		...
		DirectX::XMFLOAT4X4 LeftTransform;
		DirectX::XMFLOAT4X4 RightTransform;
    ...
    private:
		...
		Microsoft::WRL::ComPtr<ID3D11Texture2D>                     MultisampleRenderTarget;
		unsigned m_sampleSize = 0;
		...
		winrt::Windows::Graphics::Holographic::HolographicStereoTransform PrevTransform;
		bool HasPrev = false;
		winrt::Windows::Foundation::Numerics::quaternion PrevRotation;
    };
}
