...
{
    UseHolographicCameraResources<void>([&](std::map<UINT32, std::unique_ptr<CameraResources>>& cameraResourceMap)
    {
        for (HolographicCameraPose const& cameraPose : prediction.CameraPoses())
        {
#if 0
			// FIXME TODO TBD Version 18362
			HolographicCamera camera = cameraPose.HolographicCamera();
			HolographicViewConfiguration view_config = camera.ViewConfiguration();
			winrt::Windows::Foundation::Size rts = view_config.RenderTargetSize();
			rts.Width *= 2;
			rts.Height *= 2;
			view_config.RequestRenderTargetSize(rts);
#endif
...
{
	//camera.ViewportScaleFactor(1.3f);
#if 0
	auto view_config = camera.ViewConfiguration();
	auto native = view_config.NativeRenderTargetSize();
	auto current = view_config.RenderTargetSize();
#endif
...
void DX::DeviceResources::Present(HolographicFrame frame)
{
	...
	//HolographicFramePresentWaitBehavior behavior = HolographicFramePresentWaitBehavior::WaitForFrameToFinish;
	HolographicFramePresentWaitBehavior behavior = HolographicFramePresentWaitBehavior::WaitForFrameToFinish;
    ...
    HolographicFramePresentResult presentResult = frame.PresentUsingCurrentPrediction(behavior);
